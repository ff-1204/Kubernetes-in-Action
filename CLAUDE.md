...
- **kubectl 명령의 분담.** `3.1`은 **개념만** 다룹니다 — "모든 kubectl 명령은 API 요청 하나다"(`-v=8`)와 **명령형 vs 선언형**까지. **실제 명령 사용법은 4장이 전담**합니다(`apply`·`get`·`describe`는 4.2, `logs`·`exec`·`port-forward`·`cp`·`debug`는 4.3). 3.1에 명령 목록을 다시 늘어놓지 마세요.
