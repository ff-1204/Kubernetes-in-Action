...
| 05 ~ 06장 | ✅ 작성 완료 | ⬜ 없음 |
| 07장 | ✅ 작성 완료 | ✅ 질문 32문항 + 답변 |
| 08장 이후 | ⬜ 미작성 | ⬜ 없음 |

**다음 작업: 08장 ConfigMap과 Secret으로 애플리케이션 설정하기**

> **8장에서 꼭 짚을 것 — 애노테이션·레이블과의 자리 구분.**
> [7.5절](<07장. 네임스페이스와 레이블로 리소스 구성하기/7.5 애노테이션으로 정보 덧붙이기.md>)에서 "비밀은 Secret, 큰 설정은 ConfigMap"이라고 넘겼습니다. 8장 도입부에서 그 약속을 회수하면서, **메타데이터(레이블·애노테이션)와 설정 데이터(ConfigMap·Secret)의 자리가 왜 다른지**부터 정리합니다.
> * 애노테이션은 오브젝트를 **설명**하는 것, ConfigMap은 앱에 **주입**하는 것
> * Secret은 기본적으로 base64일 뿐 **암호화가 아님** — 여기서 오해가 가장 많습니다
> * 7.1의 "네임스페이스는 격리가 아니다"와 이어집니다. Secret도 네임스페이스 소속이지만 **그것만으로 안전해지지 않습니다**

> **실행 결과를 모으는 방법.** 7장에서는 필요한 `kubectl` 명령을 모은 PowerShell 스크립트를 만들어 한 번에 돌리고, 그 출력을 노트에 옮겨 넣었습니다. 명령마다 성공 사례와 **일부러 실패시키는 사례**를 함께 넣으면 에러 메시지까지 실측으로 확보됩니다. 같은 방식을 8장 이후에도 쓰면 됩니다.
