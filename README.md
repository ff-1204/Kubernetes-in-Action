...
### 시작 전에 — [1.0 쿠버네티스를 배우기 전에](<01장. 쿠버네티스 소개/1.0 쿠버네티스를 배우기 전에.md>)

원서에 없는 **보충 절**입니다. 순서대로 공부한다면 1.1보다 먼저 읽으세요. 쿠버네티스 자체가 아니라 그 아래 깔린 기초에서 막히는 것들을 모았습니다.

- **용어 지도** — 물리 서버 · 노드 · 파드 · 컨테이너 · 웹 서버가 서로 어느 층인지 (제일 많이 헷갈리는 지점)
- 웹 서버 · WAS · 백엔드 앱 · API 서버 구분 / 서버에 직접 배포하던 방식과의 대응표
- 프로세스 · 데몬 · 종료 신호 / IP · 포트 · DNS / HTTP 메서드와 상태 코드
- 리눅스 최소 지식 · YAML 문법 · 무상태 vs 상태 유지 · PowerShell과 bash 차이

맨 앞의 **자가 진단 11문항**에서 막히는 항목만 골라 읽어도 됩니다.
...
- 파일 하나 = 책의 한 개 절 (예: `1.1 쿠버네티스가 필요한 이유.md`) — 유일한 예외가 위의 `1.0`입니다
- 제목은 `# <절번호> <절 제목>`
